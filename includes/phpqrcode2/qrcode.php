...
class QRCode {
    ...
    function printHTML($size = 2) {
        ...
        for ($r = 0; $r < $this->getModuleCount(); $r++) {
            ...
            for ($c = 0; $c < $this->getModuleCount(); $c++) {
                $color = $this->isDark($r, $c)? "#5ac336" : "none";
                print('<td style="'.$style.'width:'.$size.'px;height:'.$size.'px;background-color:'.$color.'"></td>');
            }
    ...
    function getHTML($size = 2) {

        $output = '';

        $style = "border-style:none;border-collapse:collapse;margin:0px;padding:0px;";

        $output .= '<table style="'.$style.'width:'.($this->getModuleCount()*$size).'px;height:'.($this->getModuleCount()*$size).'px;margin:1em auto;">'."\n";
        ...
        for ($r = 0; $r < $this->getModuleCount(); $r++) {
            ...
            for ($c = 0; $c < $this->getModuleCount(); $c++) {
                $color = $this->isDark($r, $c)? "#5ac336" : "none";
                $output .= '    <td style="'.$style.'width:'.$size.'px;height:'.$size.'px;background-color:'.$color.';"></td>'."\n";
            }
