...
$app->post('/booking', function ($request, $response, $args) {
	...
	//if ($_POST['event-id']==30) $_POST['transport'] = true;
	
	$evn = getEvent($_POST['event_id']);
	$transport=filter_var($_POST['transport'],FILTER_VALIDATE_BOOLEAN);
	$amount = (($evn['price']+($transport ? $evn['transport'] : 0)) * intval($_POST['quantity']));
	if ($_POST['event_id']==42) $amount += round((1/493)*(7*$amount+10000));
	...
	if($amount > 20){
		...
		try {
			\Stripe\Charge::create(array(
				...
				"description" => "TKTPASS ".$evn['title'].(($transport)? " TRANSPORT":"")." x".$_POST['quantity'],
	...
	global $mysqli;
	$query = "INSERT INTO bookings (user_id, event_id, quantity, transport, mobile, bookingTime, status) VALUES (?, ?, ?, ?, ?, UNIX_TIMESTAMP(), ?);";
	$stmt = $mysqli -> prepare($query);
	$tmp=filter_var($_POST['transport'],FILTER_VALIDATE_BOOLEAN);
	$status = $amount <= 20 ? 1 : 0;
	$stmt -> bind_param("iiiisi", $_SESSION['user']['id'], $_POST['event_id'], $_POST['quantity'], $tmp, $_POST['mobile'], $status);
	$stmt -> execute();
	$stmt->close();
