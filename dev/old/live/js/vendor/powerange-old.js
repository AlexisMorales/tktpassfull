...
/**
 * Destroy Powerange instance
 *
 * @api public
 */

Powerange.prototype.destroy = function(){
...
Powerange.prototype.setRange = function(min, max) {
  if (typeof min === 'number' && typeof max === 'number' && !this.options.hideRange) {
    var minPrefix, maxPrefix, minSuffix, maxSuffix;
    if(this.options.prefix instanceof Array){
        minPrefix = this.options.prefix[0];
        maxPrefix = this.options.prefix[1];
    } else minPrefix = maxPrefix = this.options.prefix;
    if(this.options.suffix instanceof Array){
        minSuffix = this.options.suffix[0];
        maxSuffix = this.options.suffix[1];
    } else minSuffix = maxSuffix = this.options.suffix;
    this.slider.querySelector('.range-min').innerHTML = minPrefix.concat(min,minSuffix);
    this.slider.querySelector('.range-max').innerHTML = maxPrefix.concat(max,maxSuffix);
  }
};
