            ...
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <div class="quantity-row">
                  <a id="decrease-buy-quantity" class="buy-icon-button minus"><img class="buy-icon minus" src="img/minus.svg"></a>
                  <h1 id="buy-quantity" class="text-center">1</h1>
                  <a id="increase-buy-quantity" class="buy-icon-button plus"><img class="buy-icon plus" src="img/plus.svg"></a>
                </div>
                ...
                <div class="buy-phone">
                  <form>
                    <fieldset class="form-group animated">
                      <input type="text" class="form-control" id="buy-phone-number" placeholder="Mobile number">
                    </fieldset>
                  <form>
                </div>
              </div>
              <div class="modal-footer text-center">
                <button type="button" class="btn btn-primary" id="buy-book-button">£<span></span> - Go to checkout</button>
								<br/>
								<!--small id="buy-book-quantity"><span></span> tickets left</small-->
...
<!--        <script src="js/vendor/underscore.min.js"></script> -->
        ...
        <script src="js/main.js<?php echo "?r=".time();?>"></script>
        <!--script>
		    ...
		    <script async src='//www.google-analytics.com/analytics.js'></script-->
				...
				<script>
					var handler = StripeCheckout.configure({
						key: 'pk_test_6e2JAHrqsSfADzDHDMeAssBG',
						token: function(token){
							...
							$.ajax({
								...
								success: function(data, textStatus, jqXHR){
									if(data.tktpass){
										$('#buy-quantity').html(1);
										$("#buy-option-1")[0].checked = false;
										var $currEvent = slick.$slides.eq(slick.currentSlide);
										var price = parseInt($currEvent.data('price'))/100;
										$('#buy-book-button span').html(price.toFixed(2));
										$('#buy-book-button').hide();
										$('#buyModal .modal-body').slideUp();
										$('#buyModal .alert-success, #buyModal .btn-mytickets').slideDown();
									} else
								...
								error: function( jqXHR, textStatus, errorThrown){
									...
									if(res.error == "login"){
										...
										return;
									} else alert('Sorry a real error occured sending your request. Please try again later.');
								}/*,
								complete: function(){
					...
					$("#buy-book-button").on('click',function(e) {
						...
						$('#buy-phone-number').parent().removeClass('shake has-danger');
						var mobNum = $('#buy-phone-number').val().replace(/\s/g,'');
						if(!mobNum || !/^\+?\d+$/.test(mobNum) || mobNum.length < 6){
							$('#buy-phone-number').parent().addClass('shake has-danger');
							return false;
						}
						//$(this).attr('clicked',1);
						// Open Checkout with further options
						handler.open({
							name: $('.event-info-title').text(),
							description: $('.event-info-time').text(),
      				currency: "gbp",
							amount: parseInt((parseFloat($('#buy-book-button span').html())+0.25)*100),
